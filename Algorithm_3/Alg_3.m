	...
	num_effect_frequencies = zeros( num_resample, 1 );    % The first column stores the unique frequency values \tilde{K}, and the second column stores the effective sample size of resampling weight vectors
	...
	for n = 1 : 1 : num_resample
        ...
        resample_indices = randsample( resample_index_set, K, true, weight_vec );    % Sample with replacement from the selected index set K frequencies
	...
	fig_error = figure;
	semilogy( 1 : num_resample, Least_square_rec_test( :, 1 ), 'o-' );
	hold on
	semilogy( 1 : num_resample, Least_square_rec_valid( :, 1 ), 's-' );
	semilogy( 1 : num_resample, Least_square_rec_train( :, 1 ), '*-' );
	...
	if( ~use_sinint_f && ( d == 2 ) )
		...
		xlabel( '$\omega_1$' );
		ylabel( '$\omega_2$' );
		savefig( fig1, 'Omega_dist_init.fig' );
		...
		xlabel( '$\omega_1$' );
		ylabel( '$\omega_2$' );
		savefig( fig2, 'Omega_dist_final.fig' );
		...
		savefig( fig3, 'Rotate_omega_1_dist.fig' );
		...
		savefig( fig4, 'Rotate_omega_2_dist.fig' );
