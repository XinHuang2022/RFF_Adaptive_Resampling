	...
	if( use_noisy_y_data )
        s_para = 0.0025;
		y_data = y_data + s_para * randn( J, 1 );
	end
	...
	for n = 1 : 1 : num_resample
		...
		omega_new = round( omega_rw / grid_spacing ) * grid_spacing; % omega_new is a matrix with K rows and d columns
		...
		% S_mat = exp( 1i * ( x_data_train * omega_new' ) );
		S_mat = exp( 1i * ( x_data_train * omega_unique' ) );
		...
		max_iter = 100;
		% [ beta_vec, flag, relres, iter ] = pcg( @( x ) applyRegularizedMatrix( S_mat, lambda, x ), c_vec, Rel_Tol, max_iter, [], [], beta_vec_n );
		...
		% S_mat_test = exp( 1i * ( x_data_test * omega_new' ) );
		% S_mat_valid = exp( 1i * ( x_data_valid * omega_new' ) );
        S_mat_test = exp( 1i * ( x_data_test * omega_unique' ) );
		S_mat_valid = exp( 1i * ( x_data_valid * omega_unique' ) );
		Least_square_rec_test( n, 1 ) = norm( S_mat_test * beta_vec - y_data_test )^2 / norm( y_data_test )^2;
		Least_square_rec_train( n, 1 ) = norm( S_mat * beta_vec - y_data_train )^2 / norm( y_data_train )^2;
		Least_square_rec_valid( n, 1 ) = norm( S_mat_valid * beta_vec - y_data_valid )^2 / norm( y_data_valid )^2;

		select_indices = abs( beta_vec ) > epsilon;
        beta_vec_select = beta_vec( select_indices, : );
        % omega_select = omega_new( select_indices, : );
		omega_select = omega_unique( select_indices, : );
        weight_vec = abs( beta_vec_select ) / sum( abs( beta_vec_select ) );
        weight_vec_multi = abs( beta_vec ) / sum( abs( beta_vec ) );
		num_effect_frequencies( n, 1 ) = 1 / sum( weight_vec_multi.^2 );
        % num_effect_frequencies( n, 1 ) = ( sum( abs( beta_vec) ))
    ...
    zeta_mat_final = randn( K, d );
	omega_rw_final = omega_sample + delta * zeta_mat_final;
	% Define lattice grid spacing
	grid_spacing = pi / L;
	% Project points onto the lattice ( \pi / L ) * Z^d
	omega_sample_final = round( omega_rw_final / grid_spacing ) * grid_spacing;
    [ omega_final_unique, ~, ~ ] = unique( omega_sample_final, 'rows' );
	K_tilde = size( omega_final_unique, 1 );  % Number of unique frequencies
	fprintf( 'After resampling iterations, number of unique frequencies = %d\n', K_tilde );
	S_final = exp( 1i * ( x_data_train * omega_final_unique') );
	...
	S_test_final = exp( 1i * ( x_data_test * omega_final_unique' ) );
