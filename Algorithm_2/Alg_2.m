	...
	if( use_init_omega_zero )
		...
		grid_spacing = pi / L;
		omega_init = round( omega_init / grid_spacing ) * grid_spacing; % omega_new is a matrix with K rows and d columns
	end
	...
	for n = 1 : 1 : num_resample
		...
		% Display conjugate gradient solver's results
		...
		num_effect_frequencies( n, 1 ) = 1 / sum( weight_vec.^2 );
	...
	if( use_noisy_y_data )
		...
		fprintf( 'Relative squared error in noisy data: %6.5e\n', Rel_noise_error );
	end
	...
	if( ~use_sinint_f && ( d==2 ) )
		...
		xlabel( '$\omega_1$' );
		ylabel( '$\omega_2$' );
		savefig( fig_1, 'Omega_dist_init.fig' );
		...
		% sample_omega_1 = omega_final_unique * v_vec';
		...
		% sample_omega_2 = omega_final_unique * v_vec_ortho';
		...
		histogram( sample_omega_2, num_bins, 'Normalization', 'pdf', 'FaceAlpha', 0.5, 'EdgeColor', 'k');
		hold on
